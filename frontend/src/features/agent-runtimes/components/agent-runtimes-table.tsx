...
import { AgentRuntime, AgentRuntimeConnection } from '../data/schema';
...
interface DataTableProps {
  columns: ColumnDef<AgentRuntime>[];
  loading?: boolean;
  data: AgentRuntime[];
  pageInfo?: AgentRuntimeConnection['pageInfo'];
  pageSize: number;
  totalCount?: number;
  nameFilter: string;
  typeFilter: string[];
...
export function AgentRuntimesTable({
  columns,
  loading,
  data,
  pageInfo,
  ...
  const { setSelectedAgentRuntimes, setResetRowSelection, setOpen } = useAgentRuntimes();
  ...
  const [columnVisibility, setColumnVisibility] = useState<VisibilityState>(() => {
    const stored = localStorage.getItem('agent-runtimes-table-column-visibility');
    if (stored) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem('agent-runtimes-table-column-visibility', JSON.stringify(columnVisibility));
  }, [columnVisibility]);
  ...
  useEffect(() => {
    if (selectedCount === 0) {
      ...
      const selected = filteredSelectedRows.map((row) => row.original as AgentRuntime);
      setSelectedAgentRuntimes(selected);
    }
  }, [filteredSelectedRows, selectedCount, setSelectedAgentRuntimes]);

  // Clear rowSelection when data changes and selected rows no longer exist
  useEffect(() => {
    if (Object.keys(rowSelection).length > 0 && data.length > 0) {
      const dataIds = new Set(data.map((agentRuntime) => agentRuntime.id));
    ...
    <div className="flex flex-1 flex-col overflow-hidden">
      ...
      <div className="shadow-soft relative mt-4 flex-1 overflow-auto rounded-2xl border border-[var(--table-border)]">
        <div className="min-w-max">
          <Table data-testid="agent-runtimes-table" className="border-separate border-spacing-0 rounded-2xl bg-[var(--table-background)]">
